...
class ClientController {
    ...
    async verifyOtp(req, res, next) {
        ...
        try {
            ...
            return next(new ApiError(500, "An error occurred during email verification"));
        }
    }
    ...
    async getClientByPhoneNumber(req, res, next) {
        try {
            const client = await clientService.getClientByPhoneNumber(req.params.phoneNumber);
            if (!client) {
                return res.status(404).json({ message: 'Client not found' });
            }
            res.status(200).json(client);
