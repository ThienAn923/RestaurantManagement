...
class PromotionController {
  ...
  async createPromotion(req, res, next) {
    try {
      ...
      console.log("Error detected:", error);
      return next(
        new ApiError(500, "An error occurred while creating promotion")
      );
    }
  }
  ...
  async getPromotionById(req, res, next) {
    try {
      ...
      const promotionAfterInvoice =
        await prisma.PromotionAfterInvoice.findUnique({
      ...
      //This will first
      const promotionType = promotionAfterInvoice
        ? { type: "invoice", details: promotionAfterInvoice }
        : promotionAfterDish
          ? { type: "dish", details: promotionAfterDish }
          : null;

      res.status(200).json({ promotion, promotionType });
    } catch (error) {
      return next(
        new ApiError(500, "An error occurred while retrieving promotion")
      );
    }
  }
  ...
  async getAllPromotions(req, res, next) {
    try {
      ...
      const promotionsWithTypes = promotions.map((promotion) => {
        const promotionType =
          promotion.PromotionAfterInvoice.length > 0
      ...
      console.log(promotionsWithTypes);
      console.log("AHHHHHHH");

      for (const promotion of promotionsWithTypes) {
        if (promotion.promotionType && promotion.promotionType.details) {
          const dishes = await prisma.dish.findMany({
            where: { promotionID: promotion.promotionType.details.id },
            select: { name: true },
          });

          if (dishes && dishes.length > 0) {
            promotion.promotionType.details.dishes = dishes;
          }
        }
      }
  ...
  async updatePromotion(req, res, next) {
    try {
      ...
      if (promotionAfterInvoice) {
        await promotionAfterInvoiceService.updatePromotionAfterInvoice(
          promotionAfterInvoice.id,
          req.body
        );
      } else if (promotionAfterDish) {
        await promotionAfterDishService.updatePromotionAfterDish(
          promotionAfterDish.id,
          req.body
        );
      }
      res.status(200).json({ promotion, promotionType });
      ...
      return next(
        new ApiError(500, "An error occurred while updating promotion")
      );
    }
  }
  ...
  async deletePromotion(req, res, next) {
    try {
      ...
      if (promotionAfterInvoice) {
        await promotionAfterInvoiceService.deletePromotionAfterInvoice(
          promotionAfterInvoice.id
        );
      } else if (promotionAfterDish) {
        await promotionAfterDishService.deletePromotionAfterDish(
          promotionAfterDish.id
        );
      }
      ...
      return next(
        new ApiError(500, "An error occurred while deleting promotion")
      );
    }
  }
}
