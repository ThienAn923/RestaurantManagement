...
class AccountController {
  ...
  async updateAccount(req, res, next) {
    try {
      ...
      return next(
        new ApiError(500, "An error occurred while updating account")
      );
    }
  }
  ...
  async deleteAccount(req, res, next) {
    try {
      ...
      return next(
        new ApiError(500, "An error occurred while deleting account")
      );
    }
  }
  ...
  async getCurrentUser(req, res, next) {
    try {
      ...
      return next(
        new ApiError(500, "An error occurred while retrieving user information")
      );
    }
  }
  ...
  async createAccount(req, res, next) {
    if (
      !req.body?.accountUsername ||
      !req.body?.accountAuthority ||
      !req.body?.accountPassword
    ) {
      return next(
        new ApiError(
          400,
          "All fields must be filled including accountUsername, accountAuthority, and accountPassword"
        )
      );
    }
    try {
  ...
  async login(req, res, next) {
    ...
    try {
      ...
      res.status(200).json({
        ...
        user: result.account,
        name: result.account.Person.name, // Include name
        clientId: result.account.Person.clientId, // Include EmployeeID
      });
    } catch (error) {
      console.error("Error detected:", error);
  ...
  async loginClient(req, res, next) {
    ...
    try {
      const result = await accountService.loginClient(
        accountUsername,
        accountPassword
      );
      if (!result) {
