...
const bookingRouter = require("./app/route/booking.route.js");
...
app.post("/api/payment", async (req, res) => {
  ...
  const app_trans_id = `${moment().format("YYMMDD")}_${transID}`; // Tạo app_trans_id

  const order = {
    app_id: config.app_id,
    app_trans_id: app_trans_id, // Gán app_trans_id vào đơn hàng
    app_user: "user123",
    ...
    bank_code: "",
    callback_url:
      "https://ae69-2402-800-6390-b425-9974-9f5c-7b7-6021.ngrok-free.app/api/callback",
  };

  // appid|app_trans_id|app_user|amount|app_time|embeddata|item
  const data =
    config.app_id +
    "|" +
    order.app_trans_id +
  ...
  try {
    ...
    // Trả về app_trans_id cùng với kết quả thanh toán
    return res.status(200).json({
      result: result.data,
      app_trans_id: app_trans_id, // Trả về app_trans_id
    });
  } catch (error) {
